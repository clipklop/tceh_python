...
def is_game_finished(field: str) -> bool:
    """
...
def perform_move(field: str, key: str) -> str:
    """
    ...
    if key == 's' and current_position >= 12:
        raise IndexError("Sorry, can't move down")
    if key == 'd' and current_position % 4 == 3:
        raise IndexError("Sorry, can't move to the right")
    if key == 'w' and current_position < 4:
        raise IndexError("Sorry, can't move up")
    if key == 'a' and current_position % 4 == 0:
        raise IndexError("Sorry, can't move to the left")
